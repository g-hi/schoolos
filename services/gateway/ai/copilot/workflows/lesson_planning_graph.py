...
from services.gateway.ai.copilot.nodes.lesson_planning import lesson_planning_node
...
def build_lesson_planning_graph(provider: LLMProvider, workflow_context: dict | None = None):
    graph = StateGraph(SchoolOSAIState)

    async def lesson_planning_with_provider(state: SchoolOSAIState) -> SchoolOSAIState:
        return await lesson_planning_node(state, provider)
    ...
    graph.add_node("lesson_planning", lesson_planning_with_provider)
    ...
    # Route to workflow or fallback based on normalized intent.
    ...
    # If required fields are missing, stop generation and ask for clarification.
    graph.add_conditional_edges(
        "missing_information",
        route_after_missing_information,
        {
    ...
    graph.add_edge("lesson_planning", "validation")

    # Validation controls bounded retry before failing safely.
    graph.add_conditional_edges(
        "validation",
